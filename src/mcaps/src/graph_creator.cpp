    ...
    //       (temporalgraph->nodeHasMeasurement(i, params_.GPS_topic)))
    {
      ...
      if (params_.enable[1])
      {
        ...
        if (gps_it != interpolated_measures.gps_map.end())
        {
          ...
          if (rtk_odom_msg.header.stamp.toSec() < 10)
            continue;
          Eigen::Isometry3d pose;
          CovarianceMatrixd cov;
      ...
      if (params_.enable[3])
      {
        ...
        if (vo_it != interpolated_measures.vo_map.end())
        {
          ...
          if (vo_odom_msg.header.stamp.toSec() < 10)
            continue;
          Eigen::Isometry3d pose;
          CovarianceMatrixd cov;
                          ...
                          vector<int> &temporal_WO_indices)
{
  ...
  Eigen::Isometry3d velodyne_calib; // for lida
  velodyne_calib.translation() = Eigen::Vector3d(0.689, -0.612, 0.977);
  Eigen::Quaterniond q_vel;
  q_vel.x() = 0.168365;
  q_vel.y() = -.183284;
  q_vel.z() = .968419;
  q_vel.w() = .0150184;
  ...
  for (int i = 0; i < wo.size(); i++)
  {
    ...
    if (GraphCreator::getPosesDiff(pose, last_pose).translation().norm() > params_.WO_step &&
        ...
        // auto v = (VertexSE3 *)malloc(sizeof(VertexSE3)); // don't use this
        
        g2o::VertexSE3 *v = new g2o::VertexSE3; // move it to the top
        std::cout << "ASDFFASDF" << std::endl;
        v->setEstimate(pose);
        v->setId(i);
        if (verteces.size() == 0)
          v->setFixed(true);
        ...
        if (params_.enable_imu)
        {
          ...
          if (imu_msg.header.seq > 0)
          {
            ...
            e_imu_prior->setMeasurement(pose); // now
    ...
    for (int i = 0; i < temporal_WO_indices.size(); i++, iter++)
    {
      ...
      if (GraphCreator::getPosesDiff(pose, last_pose).translation().norm() > params_.SVO_step)
      {
        if (prev_SVO_iter >= 0)
        {
          ...
          {
            Eigen::Quaterniond q(diff_SVO_T.linear());
            q.x() = 0;
            q.y() = 0;
            if (q.w() < 0)
              q.z() *= -1;
            ...
            q_yaw = Eigen::AngleAxisd(0, Eigen::Vector3d::UnitX()) * Eigen::AngleAxisd(0, Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ());

            Eigen::Quaterniond q_rollpitch(diff_SVO_T.linear());
            q_rollpitch.z() = 0;
            q_rollpitch.w() = sqrt(1 - (q_rollpitch.x() * q_rollpitch.x() + q_rollpitch.y() * q_rollpitch.y()));
            Eigen::Isometry3d T_plane;
            T_plane.setIdentity();
                          ...
                          const int &start_idx)
{
  ...
  for (int i = 0; i < sw.size(); i++)
  {
    ...
    if (i > 0)
    {
      ...
      prev_wo_msg = wo[global_idx - 1];
      wo_msg = wo[global_idx];
      ...
      vo_msg = vo[global_idx];
      prev_vo_msg = vo[global_idx - 1];
